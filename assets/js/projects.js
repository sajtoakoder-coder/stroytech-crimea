/* ============================================================
   PROJECTS — данные проектов, галерея-модалка и сетка.
   Общий файл: используется на index.html (слайдер «Наши проекты»)
   и на projects.html (полная галерея). Данные — в одном месте.
   ============================================================ */
(function () {
  'use strict';

  /* ─── ДАННЫЕ ПРОЕКТОВ ───
     cover   — фото для плитки/слайда (без расширения, грузится .webp/.png)
     gallery — массив фото для листания в модалке (без расширения)
     plans   — 2D-планы по этажам
  */
  var PROJECTS = [
    {
      badge: 'Проект №1',
      title: 'Двухэтажный коттедж в Крыму',
      short: 'Одноэтажный дом с панорамными окнами и открытой террасой.',
      desc: 'Уютный двухэтажный дом с панорамными окнами и открытой террасой. Продуманная планировка объединяет гостиную и кухню в единое светлое пространство. Строительство под ключ — от проекта до финальной отделки.',
      cover: 'assets/img/Image1-000',
      gallery: ['assets/img/Image1-000'],
      plans: [
        { label: '1 этаж', src: 'assets/img/plan1-floor1-project1.webp' },
        { label: '2 этаж', src: 'assets/img/plan1-floor2-project1.webp' }
      ],
      specs: [
        ['Площадь', '120 м²'],
        ['Этажность', '1 этаж'],
        ['Материал', 'Газобетон'],
        ['Срок строительства', '5 месяцев'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №2',
      title: 'Современный коттедж в Крыму',
      short: 'Современный загородный дом с чистыми геометрическими формами.',
      desc: 'Современный загородный дом с чистыми геометрическими формами. Большие застеклённые фасады обеспечивают естественное освещение во всех комнатах. Реализован под ключ с авторским надзором.',
      cover: 'assets/img/Image20',
      gallery: ['assets/img/Image20'],
      plans: [
        { label: '1 этаж', src: 'assets/img/plan1-floor1-project2.webp' },
        { label: '2 этаж', src: 'assets/img/plan1-floor2-project2.webp' }
      ],
      specs: [
        ['Площадь', '145 м²'],
        ['Этажность', '2 этажа'],
        ['Материал', 'Кирпич'],
        ['Срок строительства', '7 месяцев'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №3',
      title: 'Одноэтажный дом в Крыму',
      short: 'Просторный двухэтажный коттедж с эксплуатируемой кровлей и гаражом.',
      desc: 'Просторный одноэтажный коттедж с эксплуатируемой кровлей и встроенным гаражом. Открытая планировка объединяет общественные зоны и спальни с собственными санузлами. Выполнен в стиле современного минимализма.',
      cover: 'assets/img/Image32-000',
      gallery: ['assets/img/Image32-000'],
      plans: [
        { label: 'План', src: 'assets/img/plan1-project3.webp' }
      ],
      specs: [
        ['Площадь', '210 м²'],
        ['Этажность', '2 этажа'],
        ['Материал', 'Монолит + газобетон'],
        ['Срок строительства', '9 месяцев'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №4',
      title: 'Коттедж на горном рельефе',
      short: 'Дом с видом на горы, фундамент на сваях под сложный рельеф.',
      desc: 'Дом в окружении природного ландшафта с видом на горы. Фундамент на сваях адаптирован под сложный рельеф участка. Архитектура вписана в природное окружение — натуральные отделочные материалы снаружи и внутри.',
      cover: 'assets/img/Image15',
      gallery: ['assets/img/Image15'],
      plans: [
        { label: '1 этаж', src: 'assets/img/plan1-floor1-project4.webp' },
        { label: '2 этаж', src: 'assets/img/plan1-floor2-project4.webp' }
      ],
      specs: [
        ['Площадь', '160 м²'],
        ['Этажность', '1.5 этажа'],
        ['Материал', 'Газобетон + дерево'],
        ['Срок строительства', '6 месяцев'],
        ['Расположение', 'Крым']
      ]
    },
    {
      /* TODO: заменить название/описание/характеристики на реальные */
      badge: 'Проект №5',
      tag: 'Строится',
      title: 'Современный двухэтажный дом',
      short: 'Объект в процессе строительства — двухэтажный дом с панорамным остеклением.',
      desc: 'Двухэтажный дом с панорамным остеклением и просторной террасой. Объект сейчас в активной фазе строительства — подробное описание и характеристики появятся позже.',
      cover: 'assets/img/project5/01',
      gallery: [
        'assets/img/project5/01',
        'assets/img/project5/02',
        'assets/img/project5/03',
        'assets/img/project5/04',
        'assets/img/project5/05',
        'assets/img/project5/06',
        'assets/img/project5/07',
        'assets/img/project5/08',
        'assets/img/project5/09',
        'assets/img/project5/10'
      ],
      plans: [],
      specs: [
        ['Статус', 'В процессе строительства'],
        ['Этажность', '2 этажа'],
        ['Площадь', '— м²'],
        ['Материал', '—'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №6',
      title: 'АЗС Red Petrol',
      short: 'Коммерческий объект — автозаправочная станция с магазином и навесом.',
      desc: 'Строительство автозаправочной станции под брендом Red Petrol: монтаж металлоконструкций навеса, возведение здания магазина с фасадом из алюминиевых композитных панелей, устройство топливных островков и благоустройство территории. Объект сдан под ключ.',
      cover: 'assets/img/project6/01',
      gallery: [
        'assets/img/project6/01',
        'assets/img/project6/project6-1st',
        'assets/img/project6/project6-2st',
        'assets/img/project6/project6-3st',
        'assets/img/project6/project6-4st'
      ],
      plans: [],
      specs: [
        ['Тип объекта', 'Коммерческий'],
        ['Площадь здания', '~180 м²'],
        ['Материал', 'Металлоконструкции + АКП'],
        ['Срок строительства', '4 месяца'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №7',
      title: 'Вилла на Южном берегу Крыма',
      short: 'Двухэтажная вилла с панорамным остеклением, бассейном и видом на море.',
      desc: 'Двухэтажная вилла на склоне с панорамным видом на море. Большое остекление объединяет интерьер с открытой террасой и бассейном. Плоская эксплуатируемая кровля, монолитный каркас, отделка штукатуркой и деревом.',
      cover: 'assets/img/project7/project7-1st',
      gallery: [
        'assets/img/project7/project7-1st'
      ],
      plans: [
        { label: '1 этаж', src: 'assets/img/project7/2dplan1.webp' },
        { label: '2 этаж', src: 'assets/img/project7/2dplan2.webp' }
      ],
      specs: [
        ['Площадь', '~220 м²'],
        ['Этажность', '2 этажа'],
        ['Материал', 'Монолит + газобетон'],
        ['Срок строительства', '10 месяцев'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №8',
      title: 'Одноэтажный дом с бассейном',
      short: 'Современный одноэтажный коттедж с деревянной террасой и бассейном.',
      desc: 'Одноэтажный коттедж в стиле современного минимализма. Деревянная терраса, выходящая к бассейну, плавно соединяет интерьер с улицей. Фасад отделан деревом и штукатуркой, большие раздвижные окна.',
      cover: 'assets/img/project8/project8-1st',
      gallery: [
        'assets/img/project8/project8-1st'
      ],
      plans: [
        { label: 'План', src: 'assets/img/project8/2dplan1.webp' }
      ],
      specs: [
        ['Площадь', '~150 м²'],
        ['Этажность', '1 этаж'],
        ['Материал', 'Монолит + дерево'],
        ['Срок строительства', '7 месяцев'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №9',
      title: 'Двухэтажный коттедж с террасой',
      short: 'Белый двухэтажный коттедж с открытой террасой, бассейном и зоной барбекю.',
      desc: 'Двухэтажный коттедж с чистым белым фасадом и панорамным остеклением. Открытая терраса объединяет зону барбекю, летнюю кухню и бассейн. Подсветка фасада и территории — авторское решение для вечернего освещения.',
      cover: 'assets/img/project9/project9-1st',
      gallery: [
        'assets/img/project9/project9-1st'
      ],
      plans: [
        { label: '1 этаж', src: 'assets/img/project9/2dplan1.webp' }
      ],
      specs: [
        ['Площадь', '~180 м²'],
        ['Этажность', '2 этажа'],
        ['Материал', 'Монолит + штукатурка'],
        ['Срок строительства', '8 месяцев'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №10',
      title: 'Коттедж с бассейном и гаражом',
      short: 'Двухэтажный коттедж с эксплуатируемой кровлей, бассейном и встроенным гаражом.',
      desc: 'Представительный двухэтажный коттедж с отделкой природным камнем. Эксплуатируемая плоская кровля с террасой и перголой, встроенный гараж, бассейн с деревянным настилом и зоной отдыха. Проект реализован под ключ.',
      cover: 'assets/img/project10/project10-1st',
      gallery: [
        'assets/img/project10/project10-1st'
      ],
      plans: [
        { label: '1 этаж', src: 'assets/img/project10/2dplan1.webp' },
        { label: '2 этаж', src: 'assets/img/project10/2dplan2.webp' }
      ],
      specs: [
        ['Площадь', '~280 м²'],
        ['Этажность', '2 этажа'],
        ['Материал', 'Монолит + природный камень'],
        ['Срок строительства', '12 месяцев'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №11',
      title: 'Трёхэтажный коттедж в Крыму',
      short: 'Трёхэтажный коттедж, реализованный под ключ в Крыму.',
      desc: 'Трёхэтажный коттедж, реализованный под ключ в Крыму. Подробная информация об объекте будет добавлена позже.',
      cover: 'assets/img/project12/project12',
      gallery: ['assets/img/project12/project12'],
      plans: [
        { label: '1 этаж', src: 'assets/img/project12/2dplan1.webp' },
        { label: '2 этаж', src: 'assets/img/project12/2dplan2.webp' },
        { label: '3 этаж', src: 'assets/img/project12/2dplan3.webp' }
      ],
      specs: [
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №12',
      title: 'Двухэтажный коттедж в Крыму',
      short: 'Двухэтажный коттедж, реализованный под ключ в Крыму.',
      desc: 'Двухэтажный коттедж, реализованный под ключ в Крыму. Подробная информация об объекте будет добавлена позже.',
      cover: 'assets/img/project13/project13',
      gallery: ['assets/img/project13/project13'],
      plans: [
        { label: '1 этаж', src: 'assets/img/project13/2dplan1st.webp' },
        { label: '2 этаж', src: 'assets/img/project13/2dplan2st.webp' }
      ],
      specs: [
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №13',
      title: 'Коттедж в Крыму',
      short: 'Коттедж, реализованный под ключ в Крыму.',
      desc: 'Коттедж, реализованный под ключ в Крыму. Подробная информация об объекте будет добавлена позже.',
      cover: 'assets/img/project14/project14',
      gallery: ['assets/img/project14/project14'],
      plans: [
        { label: 'План', src: 'assets/img/project14/2dplan.webp' }
      ],
      specs: [
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №14',
      title: 'Вигвамы Riga Village Resort',
      short: 'Уникальная пляжная зона отдыха на первой береговой линии Азовского моря в Крыму.',
      desc: 'Уникальная пляжная зона отдыха на территории Riga Village Resort, расположенная на первой береговой линии Азовского моря в Крыму. Композиция из архитектурных вигвамов с парусными навесами создает комфортное пространство для отдыха гостей отеля, проведения мероприятий и фотосессий. Легкие конструкции органично вписаны в прибрежный ландшафт и подчеркивают курортную атмосферу комплекса. Отель работает по системе «ультра всё включено» и располагает собственным оборудованным пляжем.',
      cover: 'assets/img/project15/project15',
      gallery: [
        'assets/img/project15/project15'
      ],
      plans: [],
      specs: [
        ['Назначение', 'Зона отдыха и мероприятий'],
        ['Конструкция', 'Деревянный каркас + парусные мембраны'],
        ['Статус', 'Реализовано под ключ'],
        ['Расположение', 'Riga Village Resort, Крым']
      ]
    },
    {
      badge: 'Проект №15',
      title: 'Двухэтажный дом с эксплуатируемой кровлей',
      short: 'Современный дом с акцентной кирпичной кладкой, террасой на крыше и видом на море.',
      desc: 'Двухэтажный жилой дом с акцентной кирпичной кладкой на фасаде и плоской эксплуатируемой кровлей. Верхняя терраса открывает панорамный вид на море. Первый этаж — общественные зоны с выходом на придомовую территорию, второй — приватные комнаты с балконом. Реализован под ключ.',
      cover: 'assets/img/project16/project16',
      gallery: [
        'assets/img/project16/project16',
        'assets/img/project16/project162',
        'assets/img/project16/project163',
        'assets/img/project16/project164',
        'assets/img/project16/project165',
        'assets/img/project16/project166',
        'assets/img/project16/project167'
      ],
      plans: [],
      specs: [
        ['Этажность', '2 этажа'],
        ['Материал', 'Газобетон + кирпич'],
        ['Статус', 'Реализовано под ключ'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №16',
      title: 'Бар-ресторан Riga Village Resort',
      short: 'Коммерческий объект — бар-ресторан с панорамным остеклением на территории курорта.',
      desc: 'Строительство бара-ресторана на территории Riga Village Resort. Здание с панорамным остеклением и открытой зоной, обрамлённой пальмами. Современная отделка фасада сочетает штукатурку, алюминиевые конструкции и стекло. Объект сдан под ключ.',
      cover: 'assets/img/project17/project17',
      gallery: [
        'assets/img/project17/project17'
      ],
      plans: [],
      specs: [
        ['Тип объекта', 'Коммерческий'],
        ['Назначение', 'Бар-ресторан'],
        ['Материал', 'Металлоконструкции + стекло'],
        ['Статус', 'Реализовано под ключ'],
        ['Расположение', 'Riga Village Resort, Крым']
      ]
    },
    {
      badge: 'Проект №17',
      title: 'Ресепшен Riga Village Resort',
      short: 'Здание ресепшн курортного комплекса с ландшафтным благоустройством.',
      desc: 'Строительство здания ресепшн на территории Riga Village Resort. Классический фасад с витражным остеклением и декоративной вывеской. Территория благоустроена: брусчатка, декоративные растения и живая изгородь. Объект сдан под ключ.',
      cover: 'assets/img/project18/project18',
      gallery: [
        'assets/img/project18/project18'
      ],
      plans: [],
      specs: [
        ['Тип объекта', 'Коммерческий'],
        ['Назначение', 'Ресепшен'],
        ['Статус', 'Реализовано под ключ'],
        ['Расположение', 'Riga Village Resort, Крым']
      ]
    },
    {
      badge: 'Проект №18',
      title: 'Коттеджный комплекс с бассейном',
      short: 'Курортный комплекс из нескольких домов с общим бассейном и террасной застройкой.',
      desc: 'Комплекс из нескольких двухэтажных домов на террасированном участке. Дома выполнены в едином стиле: белая штукатурка, деревянная отделка, панорамные балконы. Общая зона отдыха с бассейном, деревянным настилом и шезлонгами. Реализовано под ключ.',
      cover: 'assets/img/project19/project19',
      gallery: [
        'assets/img/project19/project19',
        'assets/img/project19/project192',
        'assets/img/project19/project193',
        'assets/img/project19/project194',
        'assets/img/project19/project195',
        'assets/img/project19/project196',
        'assets/img/project19/project197',
        'assets/img/project19/project198'
      ],
      plans: [],
      specs: [
        ['Тип объекта', 'Жилой комплекс'],
        ['Этажность', '2 этажа'],
        ['Материал', 'Каркас + дерево'],
        ['Статус', 'Реализовано под ключ'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №19',
      title: 'Коттедж с террасой и детской площадкой',
      short: 'Двухэтажный дом с деревянной отделкой, балконами и благоустроенным двором.',
      desc: 'Двухэтажный коттедж с деревянной отделкой фасада и панорамными балконами. Придомовая территория благоустроена: деревянный настил, детская площадка с батутом и озеленение. Дом расположен на террасированном участке.',
      cover: 'assets/img/project20/project20',
      gallery: [
        'assets/img/project20/project20',
        'assets/img/project20/project201'
      ],
      plans: [],
      specs: [
        ['Этажность', '2 этажа'],
        ['Материал', 'Каркас + дерево'],
        ['Статус', 'Реализовано под ключ'],
        ['Расположение', 'Крым']
      ]
    },
    {
      badge: 'Проект №20',
      title: 'Двухэтажная вилла с видом на море',
      short: 'Современная вилла с панорамным остеклением, террасами и детской площадкой.',
      desc: 'Двухэтажная вилла на склоне с панорамным видом на море и горы. Каждый этаж имеет собственную террасу с остеклением в пол. Придомовая территория благоустроена: парковка, ландшафтный дизайн и детская площадка. Реализована под ключ.',
      cover: 'assets/img/project21/project21',
      gallery: [
        'assets/img/project21/project21'
      ],
      plans: [
        { label: '1 этаж', src: 'assets/img/project21/project21-2dplan-1st.webp' },
        { label: '2 этаж', src: 'assets/img/project21/project21-2dplan-2st.webp' }
      ],
      specs: [
        ['Этажность', '3 этажа'],
        ['Материал', 'Монолит + штукатурка'],
        ['Статус', 'Реализовано под ключ'],
        ['Расположение', 'Крым']
      ]
    }
  ];

  function imgSrc(base) { return base + '.webp'; }

  /* ─── СЕТКА ПРОЕКТОВ (только на projects.html) ─── */
  var grid = document.getElementById('projectsGrid');
  if (grid) {
    PROJECTS.forEach(function (p, i) {
      var card = document.createElement('button');
      card.type = 'button';
      card.className = 'proj-card';
      card.setAttribute('data-project', i);
      card.setAttribute('aria-label', p.badge + ': ' + p.title);

      var media = document.createElement('div');
      media.className = 'proj-card__media';
      var im = document.createElement('img');
      im.src = imgSrc(p.cover);
      im.alt = p.title;
      im.loading = i < 4 ? 'eager' : 'lazy';
      im.decoding = 'async';
      if (i === 0) im.setAttribute('fetchpriority', 'high');
      media.appendChild(im);
      if (p.tag) {
        var tg = document.createElement('span');
        tg.className = 'proj-card__tag';
        tg.textContent = p.tag;
        media.appendChild(tg);
      }

      var body = document.createElement('div');
      body.className = 'proj-card__body';
      body.innerHTML =
        '<h3 class="proj-card__title"></h3>' +
        '<p class="proj-card__desc"></p>' +
        '<span class="proj-card__more">Смотреть проект ' +
        '<svg width="14" height="10" viewBox="0 0 16 10" fill="none" aria-hidden="true"><path d="M1 5h14M10 1l5 4-5 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></span>';
      body.querySelector('.proj-card__title').textContent = p.title;
      body.querySelector('.proj-card__desc').textContent = p.short || '';

      card.appendChild(media);
      card.appendChild(body);
      card.addEventListener('click', function () { openModal(i); });
      grid.appendChild(card);
    });
  }

  /* ─── МОДАЛКА ПРОЕКТА (на обеих страницах) ─── */
  var modal = document.getElementById('projectModal');
  if (!modal) return; // нет модалки на странице — дальше нечего делать

  var backdrop = modal.querySelector('.pmodal__backdrop');
  var pmodalTrack = document.getElementById('pmodalTrack');
  var pmodalBadge = document.getElementById('pmodalBadge');
  var pmodalStatus = document.getElementById('pmodalStatus');
  var pmodalTitle = document.getElementById('pmodalTitle');
  var pmodalDesc = document.getElementById('pmodalDesc');
  var pmodalPlanBtn = document.getElementById('pmodalPlanBtn');
  var pmodalClose = document.getElementById('pmodalClose');
  var galPrev = document.getElementById('pmodalGalPrev');
  var galNext = document.getElementById('pmodalGalNext');
  var galCounter = document.getElementById('pmodalGalCounter');

  var planOverlay = document.getElementById('planOverlay');
  var planImg = document.getElementById('planImg');
  var planPlaceholder = document.getElementById('planPlaceholder');
  var planClose = document.getElementById('planClose');

  var currentPlans = [];
  var currentGallery = [];
  var galIdx = 0;

  /* строим трек из img-элементов, как в хиро */
  function buildTrack(gallery, altText) {
    pmodalTrack.innerHTML = '';
    pmodalTrack.style.transition = 'none';
    pmodalTrack.style.transform = 'translateX(0)';
    gallery.forEach(function (base, idx) {
      var img = document.createElement('img');
      img.className = 'pmodal__img';
      img.src = imgSrc(base);
      img.alt = altText || '';
      img.decoding = 'async';
      if (idx === 0) img.setAttribute('fetchpriority', 'high');
      pmodalTrack.appendChild(img);
    });
  }

  function showPhoto(i) {
    if (!currentGallery.length) return;
    galIdx = (i + currentGallery.length) % currentGallery.length;
    /* включаем transition только когда уже есть слайды (не при первом открытии) */
    pmodalTrack.style.transition = 'transform .7s cubic-bezier(.4,0,.2,1)';
    pmodalTrack.style.transform = 'translateX(-' + (galIdx * 100) + '%)';
    var multi = currentGallery.length > 1;
    if (galCounter) {
      galCounter.textContent = (galIdx + 1) + ' / ' + currentGallery.length;
      galCounter.style.display = multi ? '' : 'none';
    }
    if (galPrev) galPrev.style.display = multi ? '' : 'none';
    if (galNext) galNext.style.display = multi ? '' : 'none';
  }

  function openModal(idx) {
    var p = PROJECTS[idx];
    if (!p) return;
    currentGallery = (p.gallery && p.gallery.length) ? p.gallery : [p.cover];
    buildTrack(currentGallery, p.title);
    showPhoto(0);
    pmodalBadge.textContent = p.badge;
    if (pmodalStatus) {
      pmodalStatus.textContent = p.tag || '';
      pmodalStatus.style.display = p.tag ? '' : 'none';
    }
    pmodalTitle.textContent = p.title;
    pmodalDesc.textContent = p.desc;
    currentPlans = p.plans || [];
    pmodalPlanBtn.style.display = currentPlans.length ? '' : 'none';

    modal.classList.add('is-open');
    document.body.style.overflow = 'hidden';
  }
  window.openProjectModal = openModal;

  function closeModal() {
    modal.classList.remove('is-open');
    document.body.style.overflow = '';
    closePlan();
  }

  function openPlan(floorIdx) {
    if (typeof floorIdx !== 'number') floorIdx = 0;
    var planTabsEl = document.getElementById('planTabs');

    planTabsEl.innerHTML = '';
    if (currentPlans.length > 1) {
      currentPlans.forEach(function (pl, i) {
        var btn = document.createElement('button');
        btn.textContent = pl.label;
        btn.className = 'plan-tab' + (i === floorIdx ? ' is-active' : '');
        btn.addEventListener('click', function () { openPlan(i); });
        planTabsEl.appendChild(btn);
      });
      planTabsEl.style.display = 'flex';
    } else {
      planTabsEl.style.display = 'none';
    }

    var current = currentPlans[floorIdx];
    if (current && current.src) {
      planImg.src = current.src;
      planImg.style.display = 'block';
      planPlaceholder.style.display = 'none';
    } else {
      planImg.src = '';
      planImg.style.display = 'none';
      planPlaceholder.style.display = 'flex';
    }

    if (!planOverlay.classList.contains('is-open')) {
      planOverlay.classList.add('is-open');
    }
  }

  function closePlan() {
    planOverlay.classList.remove('is-open');
  }

  if (galPrev) galPrev.addEventListener('click', function (e) { e.stopPropagation(); showPhoto(galIdx - 1); });
  if (galNext) galNext.addEventListener('click', function (e) { e.stopPropagation(); showPhoto(galIdx + 1); });

  /* Клик по слайду на главной */
  document.querySelectorAll('.slide-clickable').forEach(function (slide) {
    slide.addEventListener('click', function () {
      openModal(parseInt(slide.getAttribute('data-project'), 10));
    });
  });

  pmodalClose.addEventListener('click', closeModal);
  backdrop.addEventListener('click', closeModal);
  pmodalPlanBtn.addEventListener('click', function () { openPlan(0); });
  planClose.addEventListener('click', closePlan);
  planOverlay.querySelector('.plan-overlay__backdrop').addEventListener('click', closePlan);

  document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape') {
      if (planOverlay.classList.contains('is-open')) { closePlan(); }
      else if (modal.classList.contains('is-open')) { closeModal(); }
    } else if (modal.classList.contains('is-open') && !planOverlay.classList.contains('is-open')) {
      if (e.key === 'ArrowLeft') { showPhoto(galIdx - 1); }
      else if (e.key === 'ArrowRight') { showPhoto(galIdx + 1); }
    }
  });

  var ctaBtn = document.getElementById('pmodalCta');
  if (ctaBtn) ctaBtn.addEventListener('click', closeModal);
})();
